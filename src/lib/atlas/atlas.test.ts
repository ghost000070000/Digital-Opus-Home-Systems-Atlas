...
import { careDesk, parseMoney } from "./care.ts";
import { emptyHistory, pushHistory, redoStep, undoStep } from "./history.ts";
import { makeSnapshot, stripHeavy } from "./snapshots.ts";
import { playbookCards } from "./playbook.ts";
import { ATLAS_TEMPLATES } from "./templates.ts";
import { createEntity, createRelationship, createRoom, createSystem, emptyDocument, nextFreePosition } from "./document.ts";
...
describe("demo atlas", () => {
  it("meets dogfood size", () => {
    ...
    assert.ok((stats.floors ?? 0) >= 2);
    assert.ok((stats.contacts ?? 0) >= 2);
...
describe("phase 2 living memory", () => {
  it("migrates schema v1 into v2 fields", () => {
    const parsed = parseDocument({
      version: 1,
      meta: { name: "Legacy" },
      rooms: [{ id: "r", name: "Den" }],
      entities: [{ id: "a", name: "Lamp", roomId: "r" }],
    });
    assert.equal(parsed.doc.version, SCHEMA_VERSION);
    assert.deepEqual(parsed.doc.floors, []);
    assert.deepEqual(parsed.doc.contacts, []);
    assert.equal(parsed.doc.entities[0]?.critical, false);
    assert.equal(parsed.doc.entities[0]?.playbook, "");
    assert.ok(parsed.warnings.some((w) => /Migrated from schema v1/.test(w)));
  });

  it("lets a custom relation carry failure", () => {
    const d = emptyDocument();
    d.entities = [
      createEntity({ id: "a", name: "Pi" }),
      createEntity({ id: "b", name: "NAS" }),
    ];
    d.relationships = [
      createRelationship({ id: "r", fromId: "a", toId: "b", type: "custom", carriesFailure: true }),
    ];
    const down = traverse(d, ["b"], "downstream");
    assert.ok(down.visited.includes("a"));
    assert.equal(down.impact.get("a"), "hard");
    const silent = emptyDocument();
    silent.entities = d.entities;
    silent.relationships = [
      createRelationship({ id: "r", fromId: "a", toId: "b", type: "custom", carriesFailure: false }),
    ];
    const quiet = traverse(silent, ["b"], "downstream");
    assert.equal(quiet.visited.includes("a"), false);
  });

  it("treats recorded spares as alternatives", () => {
    const d = emptyDocument();
    d.entities = [
      createEntity({ id: "sw", name: "Core" }),
      createEntity({ id: "sp", name: "Spare", status: "spare", spareForId: "sw" }),
      createEntity({ id: "pc", name: "PC" }),
    ];
    d.relationships = [
      createRelationship({ id: "1", fromId: "pc", toId: "sw", type: "depends-on" }),
      createRelationship({ id: "2", fromId: "sp", toId: "sw", type: "backs-up" }),
    ];
    const impact = analyzeFailure(d, { failedIds: ["sw"] });
    assert.ok(impact.alternatives.some((a) => a.alternativeId === "sp"));
  });

  it("undo stack restores the previous document", () => {
    const a = emptyDocument("A");
    a.entities = [createEntity({ id: "1", name: "One" })];
    const b = emptyDocument("B");
    b.entities = [createEntity({ id: "1", name: "One" }), createEntity({ id: "2", name: "Two" })];
    let h = emptyHistory();
    h = pushHistory(h, a, 1_000);
    const step = undoStep(h, b);
    assert.ok(step);
    assert.equal(step!.doc.entities.length, 1);
    const back = redoStep(step!.history, step!.doc);
    assert.ok(back);
    assert.equal(back!.doc.entities.length, 2);
  });

  it("coalesces rapid edits into one undo step", () => {
    const a = emptyDocument("A");
    const b = emptyDocument("B");
    const c = emptyDocument("C");
    let h = emptyHistory();
    h = pushHistory(h, a, 1_000);
    h = pushHistory(h, b, 1_100);
    assert.equal(h.past.length, 1);
    h = pushHistory(h, c, 2_000);
    assert.equal(h.past.length, 2);
  });

  it("strips images from snapshots", () => {
    const d = emptyDocument("Pic");
    d.entities = [createEntity({ id: "1", name: "Cam", imageDataUrl: "data:image/png;base64,xxxx" })];
    const snap = makeSnapshot(d, "Before");
    assert.equal(snap.doc.entities[0]?.imageDataUrl, "");
    assert.equal(stripHeavy(d).entities[0]?.imageDataUrl, "");
    assert.equal(snap.entityCount, 1);
  });

  it("builds a care desk from dates and prices", () => {
    const d = emptyDocument();
    d.entities = [
      createEntity({
        id: "ups",
        name: "UPS",
        nextService: "2020-01-01",
        purchase: { date: "2019-01-01", price: "$120.50", vendor: "Shop", warrantyUntil: "2020-06-01" },
      }),
      createEntity({
        id: "nas",
        name: "NAS",
        purchase: { date: "2024-01-01", price: "500", vendor: "Shop", warrantyUntil: "" },
      }),
    ];
    const desk = careDesk(d, new Date("2026-09-12T12:00:00Z"));
    assert.equal(parseMoney("$120.50"), 120.5);
    assert.ok(desk.overdue.some((r) => r.entity.id === "ups"));
    assert.ok(desk.totalKnown > 600);
  });

  it("writes playbook cards for critical items", () => {
    const d = emptyDocument();
    d.entities = [
      createEntity({ id: "ups", name: "UPS", critical: true, playbook: "Shut the NAS down." }),
      createEntity({ id: "nas", name: "NAS" }),
    ];
    d.relationships = [createRelationship({ id: "1", fromId: "nas", toId: "ups", type: "powered-by" })];
    const cards = playbookCards(d);
    assert.ok(cards.some((c) => c.entity.id === "ups" && c.hard.some((e) => e.id === "nas")));
  });

  it("ships starter templates", () => {
    for (const t of ATLAS_TEMPLATES) {
      const doc = t.build();
      assert.ok(doc.entities.length >= 6, t.id);
      assert.ok(doc.relationships.length >= 4, t.id);
    }
  });

  it("keeps willow house floors, playbooks, and a spare", () => {
    const doc = willowHouseDemo();
    assert.ok(doc.floors.length >= 3);
    assert.ok(doc.contacts.length >= 3);
    assert.ok(doc.entities.some((e) => e.id === "ent_ups_closet" && e.playbook.length > 20));
    assert.ok(doc.entities.some((e) => e.spareForId === "ent_switch"));
    const custom = doc.relationships.find((r) => r.id === "rel_pi_nas");
    assert.equal(custom?.carriesFailure, true);
    const down = traverse(doc, ["ent_nas"], "downstream");
    assert.ok(down.visited.includes("ent_pi"));
  });
});
