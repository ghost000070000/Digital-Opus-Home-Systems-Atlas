...
export function createFloor(partial: Partial<Floor> & { name: string }): Floor {
  return {
    id: partial.id ?? uid("floor"),
    name: partial.name,
    notes: partial.notes ?? "",
...
export function createContact(partial: Partial<Contact> & { name: string }): Contact {
  return {
    id: partial.id ?? uid("who"),
    name: partial.name,
    role: partial.role ?? "",
    phone: partial.phone ?? "",
    notes: partial.notes ?? "",
...
export function createEntity(
  ...
  return {
    ...
    nextService: partial.nextService ?? "",
    playbook: partial.playbook ?? "",
    spareForId: partial.spareForId ?? null,
  };
}
...
export function createRelationship(
  ...
  return {
    ...
    carriesFailure: partial.carriesFailure ?? false,
  };
}
...
export function floorMap(doc: AtlasDocument): Map<string, Floor> {
  return new Map(doc.floors.map((f) => [f.id, f]));
}
...
export function duplicateEntityRecord(doc: AtlasDocument, id: string): Entity | null {
  const src = doc.entities.find((e) => e.id === id);
  if (!src) return null;
  return createEntity({
    ...src,
    id: uid("ent"),
