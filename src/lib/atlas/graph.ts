...
export function buildDependencyEdges(
  ...
  for (const rel of doc.relationships) {
    ...
    if (rel.type === "custom" && rel.carriesFailure) {
      edges.push({
        from: rel.fromId,
        to: rel.toId,
        relationId: rel.id,
...
export function pruneDangling(doc: AtlasDocument): AtlasDocument {
  ...
  const floorIds = new Set(doc.floors.map((f) => f.id));
  return {
    ...doc,
    rooms: doc.rooms.map((r) => ({
      ...r,
      floorId: r.floorId && floorIds.has(r.floorId) ? r.floorId : null,
    })),
    entities: doc.entities.map((e) => ({
      ...e,
      roomId: e.roomId && roomIds.has(e.roomId) ? e.roomId : null,
      systemIds: e.systemIds.filter((id) => sysIds.has(id)),
      spareForId: e.spareForId && ids.has(e.spareForId) ? e.spareForId : null,
    })),
...
export function duplicateIds(doc: AtlasDocument): string[] {
  ...
  for (const f of doc.floors) consider(f.id);
  ...
  for (const c of doc.contacts) consider(c.id);
  return dups;
}
