...
const NAV = [
  ...
  { to: "/playbook", label: "Playbook", icon: ClipboardList },
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const canRedo = useAtlas((s) => s.canRedo);
  const duplicateEntity = useAtlas((s) => s.duplicateEntity);
  const selectedEntityId = useAtlas((s) => s.ui.selectedEntityId);
  const lastNotice = useAtlas((s) => s.lastNotice);
  ...
  }, [undo, redo, duplicateEntity, selectedEntityId]);
    ...
    <TooltipProvider>
      <div className="flex h-dvh min-h-0 flex-col bg-bg text-fg">
        <header className="flex h-14 shrink-0 items-center gap-3 border-b border-border px-3 md:px-4">
          ...
          {lastNotice ? (
            <p className="hidden max-w-[180px] truncate text-[11px] text-faint lg:block">{lastNotice}</p>
          ) : null}
        ...
        <Sheet open={moreOpen} onOpenChange={setMoreOpen}>
          <SheetContent side="bottom" className="md:hidden">
            ...
            <p className="mt-1 text-sm text-muted">Cables, playbook, care, insights, labels, backup, and the guide.</p>
