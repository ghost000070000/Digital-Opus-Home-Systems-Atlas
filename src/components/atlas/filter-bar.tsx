...
export function FilterBar() {
  ...
  const setFloorFilter = useAtlas((s) => s.setFloorFilter);
    ...
    <div className="flex flex-col gap-1.5 border-b border-border bg-bg-elevated px-3 py-2">
      {doc.floors.length > 0 ? (
        <div className="flex items-center gap-1.5 overflow-x-auto">
          <span className="mr-1 shrink-0 text-[11px] uppercase tracking-[0.14em] text-faint">Floors</span>
          {doc.floors.map((f) => {
            const on = ui.floorFilter.length === 0 || ui.floorFilter.includes(f.id);
            return (
              <button
                key={f.id}
                onClick={() => toggle(ui.floorFilter, f.id, setFloorFilter)}
          ...
          {ui.floorFilter.length ? (
            <button className="shrink-0 text-xs text-copper" onClick={() => setFloorFilter([])}>
              Clear
            </button>
          ) : null}
        </div>
