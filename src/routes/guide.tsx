    ...
    <div className="min-h-0 flex-1 overflow-y-auto">
      ...
      <article className="mx-auto max-w-3xl space-y-8 px-4 py-6 text-sm leading-relaxed text-muted md:px-6">
        ...
        <section className="space-y-2">
          ...
          <p>
            Custom relations never move failure unless you turn on <em>carries failure</em> on that link.
          </p>
        </section>
        <section className="space-y-2">
          <h3 className="font-display text-2xl text-fg">Floors</h3>
          <p>
            Floors group rooms (Ground, Upper, Utility). Filter the atlas by floor. A room can live without a floor.
          </p>
        </section>
        <section className="space-y-2">
          <h3 className="font-display text-2xl text-fg">Care and playbooks</h3>
          <p>
            Next-service dates and warranties live on each item. Care lists what is overdue. Playbook builds response
            cards for critical items and single points of failure. People you add there stay in this browser.
          </p>
        </section>
        <section className="space-y-2">
          <h3 className="font-display text-2xl text-fg">Undo and snapshots</h3>
          <p>
            ⌘Z / Ctrl+Z undoes the last edit. Backup can freeze a named snapshot of the atlas. Restoring a snapshot is
            itself undoable. Snapshots drop attached images to stay small.
          </p>
        </section>
        <section className="space-y-2">
        ...
        <section className="space-y-2">
          ...
          <p>
            ...
            <kbd className="rounded-sm bg-surface px-1.5 py-0.5 font-mono text-xs">⌘K</kbd> search.{" "}
            <kbd className="rounded-sm bg-surface px-1.5 py-0.5 font-mono text-xs">⌘Z</kbd> undo.{" "}
            <kbd className="rounded-sm bg-surface px-1.5 py-0.5 font-mono text-xs">⌘⇧Z</kbd> redo.{" "}
            <kbd className="rounded-sm bg-surface px-1.5 py-0.5 font-mono text-xs">⌘D</kbd> duplicate the selected
            item. Drag the atlas
